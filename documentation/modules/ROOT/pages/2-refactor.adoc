= 2. Refactor - 25 minutes
:imagesdir: ../assets/images

== Goals of this lab

The goal is to understand the modernization issues of the `customers` application and learn how you can fix them by the code modification. The Tackle Analysis report also provides the details in terms of the *file name*, *path*, and *solution recommendations* for each issue. 

In this lab you will also analyze the customers application once again using Tackle Analysis when the all issue are fixed.
...
Access link:https://console-openshift-console.apps.%SUBDOMAIN%/topology/ns/retail-%USERID%?view=graph[Topology view^] of reatail-%USERID% project in Developer perspective. You will see the `oracle-database` virtual machine on OpenShift.
...
Go to the `Terminal` window in VS Code server. Then, build and package the customers application again:

[.console-input]
[source,sh,subs="+attributes,macros+"]
----
cd $HOME/app-mod-projects/customers-tomcat-legacy/ &&
mvn clean package
----
...
[INFO] Assembling webapp [customers-tomcat] in [/home/codeserver/app-mod-projects/customers-tomcat-legacy/target/customers-tomcat-0.0.1-SNAPSHOT]
[INFO] Processing war project
[INFO] Building war: /home/codeserver/app-mod-projects/customers-tomcat-legacy/target/customers-tomcat-0.0.1-SNAPSHOT.war
[INFO] ------------------------------------------------------------------------
[INFO] BUILD SUCCESS
[INFO] ------------------------------------------------------------------------
...
Let's go back to the link:https://tackle-tackle-%USERID%.apps.%SUBDOMAIN%/applications/analysis-tab[Tackle Analysis web console^]. Then, create a new inventory to analyze th modernized application (*customers*). 
...
[NOTE]
====
You can use the solution artifact (`customers-tomcat-solution.war`) that is packaged with the fixed code in your link:https://gitea.apps.%SUBDOMAIN%/%USERID%/app-mod-projects[Gitea repository^] rather than walking through multiple steps from _commit_ to _push_, and _pull_ the new application artifact (_.war_ file) to your local file system..
...
You will now be presented with options for transformation targets. Click on `Containers`, `Linux`, and `OpenJDK` as same targets of the legacy application.
