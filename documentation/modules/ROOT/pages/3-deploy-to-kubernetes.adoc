= 3. Deploy to Kubernetes - 25 minutes
...
== 3.1. Update Oracle DMBS Connection using Helm

=== 3.1.1. Why OpenShift with Helm?
...
=== 3.1.2. Update the Helm Chart for a new JDBC configuration
...
== 3.2. Run an OpenShift Pipelines

=== 3.2.1 Why OpenShift Pipelines?
...
=== 3.2.2 Execute the Customers Pipelines
...
=== 3.2.3 Add Labels for better Topology View
...
== 3.3. Sync the Gateway application in ArgoCD

=== 3.3.1 Why OpenShift GitOps?
...
=== 3.3.2 Update the Gateway Configuration
...
=== 3.3.3 Sync the configuration change by ArgoCD
